...
class World implements ChunkManager{
		...
		if(($blockSound = ($sound instanceof BlockSound)) || $sound instanceof ProtocolSound){
			if($blockSound){
				$closure = function(TypeConverter $typeConverter) use ($sound, $pos) : array{
					...
					//BlockSound::toRuntimeId() needs the protocol ID too, to route protocol 223
					//through Legacy223BlockRuntimeIdMap instead of the modern BlockTranslator - see
		...
		if($particle instanceof BlockParticle || $particle instanceof ItemParticle || $particle instanceof ProtocolParticle){
			$closure = function(TypeConverter $typeConverter) use ($particle, $pos) : array{
				if($particle instanceof ItemParticle){
					...
					//BlockParticle::toRuntimeId() needs the protocol ID too, to route protocol 223
					//through Legacy223BlockRuntimeIdMap instead of the modern BlockTranslator - see
					//Legacy223BlockRuntimeIdMap::resolveNetworkBlockRuntimeId() doc comment.
	 ...
	 */
	public function createBlockUpdatePackets(TypeConverter|array $typeConverter, ?array $blocks = null) : array{
		if(is_array($typeConverter)){
			$blocks = $typeConverter;
			$typeConverter = TypeConverter::getInstance();
		}
		if($blocks === null){
			throw new \InvalidArgumentException("\$blocks must be provided");
		}

		$packets = [];

		$blockTranslator = $typeConverter->getBlockTranslator();
		//Protocol 223 (1.2.13) predates the modern block-palette system entirely. The per-protocol
		//BlockTranslator above is loaded from BlockTranslator::PATHS[PROTOCOL_1_2_13], which - for lack of
		//any actual 1.2.13 block data in the bundled bedrock-data vendor package - falls back to the
		//"-1.20.0" canonical block state table. Its internalIdToNetworkId() therefore returns *modern*
		//(1.20.0-era) runtime IDs, in the thousands-to-tens-of-thousands range, from a completely different
		//palette than what a real 1.2.13 client understands.
		//
		//That's not a problem for full chunk sends (ChunkSerializer::serializeFullChunk223() uses
		//LegacyBlockStateIdMap directly and writes flat legacy id+meta arrays, with no runtime ID
		//involved), but every *individual* block update - including the break-resync path in
		//InGamePacketHandler::syncBlocksNearby() - goes through this function. Without translation it would
		//feed the client an out-of-range index into its own small (~2070-entry) hardcoded runtime ID table,
		//which resolves to an unrelated block or nothing renderable at all - visible as blocks rendering as
		//the wrong type, or an oversized/irregular gap appearing around a block that was just broken.
		//
		//Legacy223BlockRuntimeIdMap resolves the real (id, meta) -> runtimeID mapping that a genuine 1.2.13
		//client has compiled in, so chunk sends and per-block updates share the same LegacyBlockStateIdMap
		//source of truth for the legacy (id, meta) pair; only the final encoding step differs, since the
		//wire formats differ (flat array vs runtime ID), not the block identity itself.
		$isLegacy223 = $typeConverter->getProtocolId() === ProtocolInfo::PROTOCOL_1_2_13;
		$legacy223RuntimeIdMap = $isLegacy223 ? Legacy223BlockRuntimeIdMap::getInstance() : null;
		...
		foreach($blocks as $b){
			...
			if(!$isLegacy223 && $tile instanceof Spawnable){
				//This render-update-bug workaround fabricates a fake *modern* blockstate (via the modern
				//BlockStateDictionary/BlockStateData NBT machinery) to force affected clients to redraw a
				//tile. Protocol 223 has no blockstate NBT concept on the wire at all, so this workaround
				//cannot apply to it - skip entirely rather than feed legacy clients modern state data.
			...
			if($isLegacy223 && $legacy223RuntimeIdMap !== null){
				[$legacyId, $legacyMeta] = LegacyBlockStateIdMap::getInstance()->toLegacy($fullBlock->getStateId());
				$networkBlockId = $legacy223RuntimeIdMap->toRuntimeId($legacyId, $legacyMeta);
			}else{
				$networkBlockId = $blockTranslator->internalIdToNetworkId($fullBlock->getStateId());
			}
			$packets[] = UpdateBlockPacket::create(
				$blockPosition,
				$networkBlockId,
			...
			if(
				$tile instanceof Spawnable &&
				!($typeConverter->getProtocolId() === ProtocolInfo::PROTOCOL_1_2_13 && ($tile instanceof Campfire || $tile instanceof ItemFrame || $tile instanceof Jukebox || $tile instanceof Lectern))
			){
