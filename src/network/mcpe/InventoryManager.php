...
class InventoryManager{
	...
	public function addRawPredictedSlotChanges(array $networkInventoryActions) : void{
		foreach($networkInventoryActions as $action){
			...
			//
			//Protocol 223 (1.2.13) is a further, deliberate exception to that rule. That client's
			//classic PC-style inventory UI represents "pick an item up" as a SOURCE_CONTAINER action against
			//ContainerIds::UI (124) at netSlotId UIInventorySlotOffset::CURSOR (0) - moving the stack onto the
			//player's cursor - which is the first of two steps for every drag/drop gesture that client has,
			//including dropping an item into the world (step two, dropping outside the inventory, sends a
			//second transaction that never arrives if step one is rejected here). This is a real, permanent,
			//per-player inventory in this codebase (Player::$cursorInventory, registered in InventoryManager's
			//constructor via addComplex(UIInventorySlotOffset::CURSOR, ...)), and locateWindowAndSlot() already
			//has a working special case that resolves ContainerIds::UI + a complex-mapped netSlotId to it - so
			//unlike every other windowId this legacy client might send, this one both corresponds to a real
			//inventory and is already fully supported by the rest of this class and by
			//InGamePacketHandler::createLegacy223InventoryAction(). Rejecting it here was purely an oversight:
			//it threw a fatal PacketHandlingException (killing the whole connection) before any of that
			//already-correct handling ever ran, which is what made every attempt to pick up/drop an item on a
			//protocol 223 client disconnect the player outright.
			if(
				!($action->windowId === ContainerIds::UI && $this->session->getProtocolId() === ProtocolInfo::PROTOCOL_1_2_13)
				&& match($action->windowId){
	...
	/**
	 * Protocol 223 (1.2.13/1.2.16) predates CreativeContentPacket; the legacy client instead fills its creative
	 * tab from an InventoryContentPacket sent for windowId=ContainerIds::LEGACY_CREATIVE, with one entry per
	 * creative item and no group/category metadata (that concept didn't exist yet). Mirrors the pre-1.12
	 * PlayerInventory::sendCreativeContents() behaviour.
	 *
	 * Sending this packet used to reliably freeze or crash the 1.2.13 client at spawn. The cause was that
	 * CommonTypes::getItemStackWrapper()/putItemStackWrapper() had no protocol-223-specific wire format at
	 * all, so every item (id/count/meta encoding, extra data framing) was serialized in the modern layout
	 * the legacy client can't parse - hundreds of malformed items in one packet is exactly what would
	 * corrupt the client's parser and freeze it. That's fixed via getItemStackLegacy223()/
	 * putItemStackLegacy223(), and this is now called normally from PreSpawnPacketHandler; it has not yet
	 * been confirmed against a real client.
	 */
	public function syncLegacyCreative() : void{
		$typeConverter = $this->session->getTypeConverter();
		$itemTranslator = $typeConverter->getItemTranslator();
		$items = [];
		foreach($this->player->getCreativeInventory()->getAllEntries() as $entry){
			$item = $entry->getItem();
			//Unlike coreItemStackToNet() (used for real inventory contents, where an unmapped item must still be
			//displayed as *something*), the legacy creative tab has no such requirement: only items that genuinely
			//existed in the 1.2.13-1.2.16 era have a real mapping here. Falling back to INFO_UPDATE for every
			//item PMMP added since then would flood protocol 223's fixed-era creative tab with hundreds of
			//indistinguishable "unknown item" placeholders (same block ID/meta, differing only in hidden NBT the
			//legacy client's UI was never built to de-duplicate by), which is what crashed the client immediately
			//after spawning. Skip anything without a genuine mapping instead of substituting a placeholder.
			if($itemTranslator->toNetworkIdQuiet($item) === null){
				continue;
			}
			$items[] = ItemStackWrapper::legacy($typeConverter->coreItemStackToNet($item));
		}
		$this->session->sendDataPacket(InventoryContentPacket::create(
			ContainerIds::LEGACY_CREATIVE,
			$items,
