...
use pocketmine\multiversion\Compatibility\CompatibilityFactory;
use pocketmine\multiversion\Compatibility\ProtocolCompatibility;
use pocketmine\nbt\tag\CompoundTag;
use pocketmine\nbt\tag\StringTag;
...
use pocketmine\network\mcpe\compression\ZlibCompressorLegacy;
...
class NetworkSession{
	...
	private ProtocolCompatibility $compatibility;
	...
	/**
	 * Lightweight, capped diagnostics for the legacy 1.2.13-1.2.16 client protocol (wire protocol 223).
	 * These clients share one wire format across that whole version range, so we track them together
	 * under the "legacy protocol" label instead of dumping raw packet bytes to the log.
	 */
	private int $legacyProtocolChunksSent = 0;
	private int $legacyProtocolScoreboardPacketsSent = 0;
	private float $legacyProtocolScoreboardSummaryLastLoggedAt = 0.0;
	private int $legacyProtocolUnknownPacketsDropped = 0;
	private const LEGACY_PROTOCOL_CHUNK_LOG_LIMIT = 5;
	private const LEGACY_PROTOCOL_SCOREBOARD_LOG_INTERVAL_SECONDS = 30.0;
	...
	public function __construct(
		...
		//matches the CURRENT_PROTOCOL fallback used by getProtocolId() below, until setProtocolId() is called
		//with whatever the client actually negotiates during login
		$this->compatibility = CompatibilityFactory::get(ProtocolInfo::CURRENT_PROTOCOL);
	...
	public function setProtocolId(int $protocolId) : void{
		...
		$this->compatibility = CompatibilityFactory::get($protocolId);
		$this->logger->debug("Negotiated protocol {$protocolId} -> compatibility profile: " . $this->compatibility->getLabel());
		...
		//legacy 1.2.13-1.2.16 clients expect zlib-header compressed batches (see ZlibCompressorLegacy), not raw deflate
		if($this->isLegacyProtocol()){
			$this->compressor = ZlibCompressorLegacy::getInstance();
		}
	}

	/**
	 * True for clients on the legacy 1.2.13-1.2.16 Bedrock protocol (wire protocol 223), which needs
	 * several compatibility shims (compression format, wire ID collisions, log-flood-prone packets).
	 *
	 * @deprecated New code should ask $session->getCompatibility()->supports(...) for a specific capability
	 * instead of branching on "is this the legacy protocol". This method remains for the compression-format
	 * check right below it, which is a property of the protocol itself rather than a capability.
	 */
	private function isLegacyProtocol() : bool{
		return $this->protocolId === ProtocolInfo::PROTOCOL_1_2_13;
	}
	...
	/**
	 * The compatibility profile for this session's negotiated protocol - what the connected client can and
	 * can't do. Prefer this over comparing getProtocolId() against ProtocolInfo constants directly wherever
	 * the check is about a *capability* rather than a protocol-specific implementation detail.
	 */
	public function getCompatibility() : ProtocolCompatibility{
	...
	public function handleEncoded(string $payload) : void{
		...
		try{
			...
			}elseif($this->protocolId === null){
				try{
					Timings::$playerNetworkReceiveDecompress->startTiming();
					try{
						$candidate = $this->compressor->decompress($payload);
						$candidateStream = new ByteBufferReader($candidate);
						$firstPacketBuffer = null;
						foreach(PacketBatch::decodeRaw($candidateStream) as $buffer){
							$firstPacketBuffer = $buffer;
							break;
						}
						if($firstPacketBuffer === null || $this->packetPool->getPacket($firstPacketBuffer) === null){
							throw new DecompressionException("Decompressed payload did not contain a recognised packet");
						}
					}catch(DecompressionException|DataDecodeException|PacketDecodeException $e){
	...
	private function detectLegacyLoginProtocolId(Packet $packet, string $buffer) : ?int{
		if(!($packet instanceof LoginPacket)){
			return null;
		}

		return LoginPacket::sniffLegacyProtocolId($buffer);
	}
	...
	public function handleDataPacket(Packet $packet, string $buffer) : void{
		...
		try{
			...
			try{
				...
				$protocolIdForDecode = $this->protocolId === null
					? ($this->detectLegacyLoginProtocolId($packet, $buffer) ?? $this->getProtocolId())
					: $this->getProtocolId();
				try{
					$packet->decode($stream, $protocolIdForDecode);
	 ...
	 */
	/** Packets whose wire ID collides with a different packet under the legacy protocol - never send these. */
	private const LEGACY_PROTOCOL_WIRE_COLLISION_CLASSES = [
		ServerPlayerPostMovePositionPacket::class,
		TickSyncPacket::class,
		LegacyTelemetryEventPacket::class,
		AutomationClientConnectPacket::class,
	];

	/**
	 * High-frequency scoreboard "aSync"/ping traffic on the legacy protocol. These are counted and
	 * summarized periodically (see legacyProtocolScoreboardPacketsSent) instead of being logged individually.
	 */
	private const LEGACY_PROTOCOL_HIGH_FREQUENCY_CLASSES = [
		SetDisplayObjectivePacket::class,
		SetScorePacket::class,
		RemoveObjectivePacket::class,
	];

	/**
	 * Highest packet wire ID that exists at all under protocol 223 (SetDefaultGameTypePacket, 0x69), confirmed
	 * against a reference protocol-223 server implementation. Every packet class added to the protocol after
	 * 1.2.16 (e.g. NetworkChunkPublisherUpdatePacket, ScoreboardIdentity, ItemStackRequest, PlayerAuthInput, ...)
	 * has a NETWORK_ID above this. The legacy client has no handler for those IDs and, unlike the modern client,
	 * has no guaranteed graceful "unknown packet" fallback - so this is a last-resort safety net catching any
	 * call site (existing or future) that forgets to gate a modern-only packet behind isLegacyProtocol(), on top
	 * of the explicit per-call-site guards elsewhere in this class.
	 */
	private const LEGACY_PROTOCOL_MAX_KNOWN_PACKET_ID = 0x69;

	/**
	 * The legacy 1.2.13-1.2.16 client predates the scoreboard feature entirely (no SetDisplayObjective/SetScore/
	 * RemoveObjective packet IDs exist on that protocol at all - confirmed against a reference protocol-223 server
	 * implementation). Instead of just dropping scoreboard traffic, we track sidebar objective state per legacy
	 * session and render it as a persistent action bar, so plugins using the normal scoreboard API get a visible
	 * (if simpler) equivalent on legacy clients instead of nothing.
	 */
	private const LEGACY_SCOREBOARD_REFRESH_INTERVAL_SECONDS = 3.0;
	private const LEGACY_SCOREBOARD_MAX_LINES = 15;

	private ?string $legacyScoreboardObjectiveName = null;
	private string $legacyScoreboardDisplayName = "";
	private int $legacyScoreboardSortOrder = SetDisplayObjectivePacket::SORT_ORDER_ASCENDING;
	/** @var array<int, array{label: string, score: int}> */
	private array $legacyScoreboardEntries = [];
	private float $legacyScoreboardLastRefreshedAt = 0.0;
	...
	private function sendDataPacketInternal(ClientboundPacket $packet, bool $immediate, ?PromiseResolver $ackReceiptResolver) : bool{
		...
		if($this->isLegacyProtocol() && in_array($packet::class, self::LEGACY_PROTOCOL_WIRE_COLLISION_CLASSES, true)){
			throw new AssumptionFailedError(get_class($packet) . " shares its wire ID with a different packet on the legacy 1.2.13-1.2.16 protocol");
		}
		if($this->isLegacyProtocol() && $this->handleLegacyScoreboardSubstitute($packet)){
			//consumed by the legacy scoreboard bridge (state updated and/or substitute already dispatched) -
			//the real packet is never forwarded, since the legacy client has no parser for it at all
			return false;
		}
		if($this->isLegacyProtocol() && $packet->pid() > self::LEGACY_PROTOCOL_MAX_KNOWN_PACKET_ID){
			if($this->legacyProtocolUnknownPacketsDropped < self::LEGACY_PROTOCOL_CHUNK_LOG_LIMIT){
				$this->legacyProtocolUnknownPacketsDropped++;
				$this->logger->debug("[legacy protocol] dropped " . get_class($packet) . " (wire ID 0x" . dechex($packet->pid()) . "): does not exist on protocol 223");
			}
			return false;
		}
		...
		try{
			...
			foreach($packets as $evPacket){
				...
				$encoded = self::encodePacketTimed($writer, $this->getProtocolId(), $evPacket);
				if($this->isLegacyProtocol() && in_array($evPacket::class, self::LEGACY_PROTOCOL_HIGH_FREQUENCY_CLASSES, true)){
					$this->legacyProtocolScoreboardPacketsSent++;
					$now = microtime(true);
					if($now - $this->legacyProtocolScoreboardSummaryLastLoggedAt >= self::LEGACY_PROTOCOL_SCOREBOARD_LOG_INTERVAL_SECONDS){
						$this->logger->debug("[legacy protocol] scoreboard packets sent in last " . self::LEGACY_PROTOCOL_SCOREBOARD_LOG_INTERVAL_SECONDS . "s: " . $this->legacyProtocolScoreboardPacketsSent);
						$this->legacyProtocolScoreboardPacketsSent = 0;
						$this->legacyProtocolScoreboardSummaryLastLoggedAt = $now;
					}
				}
	...
	/**
	 * Bridges scoreboard traffic to a substitute the legacy 1.2.13-1.2.16 client can actually render.
	 * Returns true if the packet was one of the scoreboard classes (i.e. it was consumed, whether or not it
	 * resulted in a visible change), false for anything else so the caller falls through to normal handling.
	 */
	private function handleLegacyScoreboardSubstitute(ClientboundPacket $packet) : bool{
		if($packet instanceof SetDisplayObjectivePacket){
			if($packet->displaySlot !== SetDisplayObjectivePacket::DISPLAY_SLOT_SIDEBAR){
				//no legacy substitute attempted for the tab list / below-name-tag slots
				return true;
			}
			$this->legacyScoreboardObjectiveName = $packet->objectiveName;
			$this->legacyScoreboardDisplayName = $packet->displayName;
			$this->legacyScoreboardSortOrder = $packet->sortOrder;
			$this->legacyScoreboardEntries = [];
			$this->sendLegacyScoreboardSubstitute();
			return true;
		}
		if($packet instanceof SetScorePacket){
			if($this->legacyScoreboardObjectiveName === null){
				return true; //not currently tracking a sidebar objective - nothing to render
			}
			foreach($packet->entries as $entry){
				if($entry->objectiveName !== $this->legacyScoreboardObjectiveName){
					continue;
				}
				if($packet->type === SetScorePacket::TYPE_REMOVE){
					unset($this->legacyScoreboardEntries[$entry->scoreboardId]);
					continue;
				}
				$this->legacyScoreboardEntries[$entry->scoreboardId] = [
					"label" => $this->resolveLegacyScoreboardEntryLabel($entry),
					"score" => $entry->score
				];
			}
			$this->sendLegacyScoreboardSubstitute();
		...
		if($packet instanceof RemoveObjectivePacket){
			if($this->legacyScoreboardObjectiveName === $packet->objectiveName){
				$this->legacyScoreboardObjectiveName = null;
				$this->legacyScoreboardEntries = [];
				$this->sendDataPacket(SetTitlePacket::actionBarMessage(""));
			}
			return true;
		}
		return false;
	...
	/**
	 * Fake-player entries (the common technique scoreboard plugins use to display arbitrary text lines) carry
	 * their own label directly. Real player/entity entries have no label in the packet at all - the modern client
	 * looks their name up client-side from the entity list - so here we do a best-effort match against currently
	 * online players by entity ID instead.
	 */
	private function resolveLegacyScoreboardEntryLabel(ScorePacketEntry $entry) : string{
		...
		foreach($this->server->getOnlinePlayers() as $onlinePlayer){
			if($onlinePlayer->getId() === $entry->actorUniqueId){
	...
	private function sendLegacyScoreboardSubstitute() : void{
		if($this->legacyScoreboardObjectiveName === null){
			return;
		}
		$entries = $this->legacyScoreboardEntries;
		uasort($entries, fn(array $a, array $b) : int => $this->legacyScoreboardSortOrder === SetDisplayObjectivePacket::SORT_ORDER_ASCENDING ?
			$a["score"] <=> $b["score"] :
			$b["score"] <=> $a["score"]
		);

		$lines = [$this->legacyScoreboardDisplayName];
		$lineCount = 0;
		foreach($entries as $entry){
			if(++$lineCount > self::LEGACY_SCOREBOARD_MAX_LINES){
				break;
			}
			$lines[] = $entry["label"] . ": " . $entry["score"];
		}

		$this->sendDataPacket(SetTitlePacket::actionBarMessage(implode("\n", $lines)));
		$this->legacyScoreboardLastRefreshedAt = microtime(true);
	}
	...
	private function setAuthenticationStatus(bool $authenticated, bool $authRequired, Translatable|string|null $error, ?string $clientPubKey) : void{
		...
		//protocol 223 (1.2.13) clients never send a ClientToServerHandshakePacket back after receiving
		//ours: the connection just sits at "Connecting to external server" until the user gives up.
		//Skip the encryption handshake for this protocol and go straight into the resource pack phase,
		//same as when encryption is disabled server-wide.
		if(EncryptionContext::$ENABLED && !$this->isLegacyProtocol()){
	...
	public function notifyTerrainReady() : void{
		if($this->isLegacyProtocol()){
			$this->logger->debug("[legacy protocol] sending PlayStatusPacket(PLAYER_SPAWN), total chunks sent: {$this->legacyProtocolChunksSent}");
		}
	...
	public function syncViewAreaCenterPoint(Vector3 $newPos, int $viewDistance) : void{
		//NetworkChunkPublisherUpdatePacket (wire ID 0x79) postdates protocol 223 by a wide margin - the legacy
		//1.2.13-1.2.16 client has no packet ID that high and has no concept of a "publisher" gate at all: it
		//renders each chunk as soon as the corresponding FullChunkDataPacket arrives. This runs every time the
		//player's chunk-load queue changes (i.e. continuously while walking near a chunk border), so sending it
		//unconditionally was corrupting/derailing the legacy client's packet stream progressively during normal
		//movement - the "moves fine, then crashes after a while" symptom. Skip it entirely for legacy sessions.
		if($this->isLegacyProtocol()){
			return;
		}
	...
	public function syncGameMode(GameMode $mode, bool $isRollback = false) : void{
		...
		if(!$isRollback){
			$this->syncCreativeInventory();
		}
	}

	/**
	 * syncCreative() builds a CreativeContentPacket, which does not exist on protocol 223 and is silently
	 * dropped by sendDataPacket()'s per-protocol packet filter - leaving the legacy client with no creative
	 * items. The legacy substitute is syncLegacyCreative() (InventoryContentPacket, windowId=
	 * LEGACY_CREATIVE), which PreSpawnPacketHandler already sends correctly at spawn time. Any other place
	 * that needs to (re-)send the creative tab after spawn - a post-spawn gamemode change
	 * (see syncGameMode()) or a plugin swapping the player's creative inventory via
	 * Player::setCreativeInventory() - must go through this method instead of calling
	 * InventoryManager::syncCreative() directly, or it silently regresses this same bug for protocol 223.
	 */
	public function syncCreativeInventory() : void{
		if($this->invManager === null){
			return;
		}
		if($this->isLegacyProtocol()){
			$this->invManager->syncLegacyCreative();
		}else{
	...
	public function syncAbilities(Player $for) : void{
		...
		if($this->isLegacyProtocol()){
			$this->sendLegacyAdventureSettings($for, $isOp);
			return;
		}
	...
	public function syncAdventureSettings() : void{
		...
		if($this->isLegacyProtocol()){
			//everything is carried by the single AdventureSettingsPacket sent from syncAbilities() on this protocol
			return;
		}
	...
	/**
	 * Legacy 1.2.13-1.2.16 clients have no UpdateAbilitiesPacket/UpdateAdventureSettingsPacket split; a
	 * single AdventureSettingsPacket carries both the fly/noclip/build-and-mine abilities and the
	 * world-immutable/no-pvp/auto-jump flags. Without this, the client never receives its permission
	 * state and gets stuck on the loading/spawn screen unable to move.
	 */
	private function sendLegacyAdventureSettings(Player $for, bool $isOp) : void{
		$flags = AdventureSettingsPacket::AUTO_JUMP;
		if($for->getAllowFlight()){
			$flags |= AdventureSettingsPacket::ALLOW_FLIGHT;
		}
		if($for->isFlying()){
			$flags |= AdventureSettingsPacket::FLYING;
		}
		if(!$for->hasBlockCollision()){
			$flags |= AdventureSettingsPacket::NO_CLIP;
		}

		$flags2 = 0;
		if(!$for->isSpectator()){
			$flags2 |= AdventureSettingsPacket::BUILD_AND_MINE
				| AdventureSettingsPacket::DOORS_AND_SWITCHES
				| AdventureSettingsPacket::OPEN_CONTAINERS
				| AdventureSettingsPacket::ATTACK_PLAYERS
				| AdventureSettingsPacket::ATTACK_MOBS;
		}
		if($isOp){
			$flags2 |= AdventureSettingsPacket::OPERATOR;
		}
		if($for->hasPermission(DefaultPermissionNames::COMMAND_TELEPORT_SELF)){
			$flags2 |= AdventureSettingsPacket::TELEPORT;
		}

		$this->sendDataPacket(AdventureSettingsPacket::create(
			$flags,
	...
	private function sendChunkPacket(string $chunkPacket, \Closure $onCompletion, World $world, int $chunkX = 0, int $chunkZ = 0) : void{
		...
		try{
			if($this->isLegacyProtocol()){
				$this->legacyProtocolChunksSent++;
				if($this->legacyProtocolChunksSent <= self::LEGACY_PROTOCOL_CHUNK_LOG_LIMIT){
					$this->logger->debug("[legacy protocol] sent compressed chunk x=$chunkX z=$chunkZ bytes=" . strlen($chunkPacket) . " (total sent: {$this->legacyProtocolChunksSent})");
				}
			}
	...
	public function startUsingChunk(int $chunkX, int $chunkZ, \Closure $onCompletion) : void{
		...
		if($this->isLegacyProtocol() && $this->legacyProtocolChunksSent < self::LEGACY_PROTOCOL_CHUNK_LOG_LIMIT){
			$this->logger->debug("[legacy protocol] queued chunk x=$chunkX z=$chunkZ");
		}
	...
	public function tick() : void{
		...
		if(
			$this->legacyScoreboardObjectiveName !== null &&
			microtime(true) - $this->legacyScoreboardLastRefreshedAt >= self::LEGACY_SCOREBOARD_REFRESH_INTERVAL_SECONDS
		){
			//the action bar fades client-side after a few seconds - resend periodically to keep the scoreboard
			//substitute visible for as long as the objective stays active
			$this->sendLegacyScoreboardSubstitute();
		}
