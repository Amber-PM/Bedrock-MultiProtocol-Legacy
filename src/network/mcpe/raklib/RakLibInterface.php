...
class RakLibInterface implements ServerEventListener, AdvancedNetworkInterface{
	...
	/**
	 * Older RakNet wire protocol versions that should still be allowed to complete the RakLib handshake,
	 * in addition to MCPE_RAKNET_PROTOCOL_VERSION above. This does NOT affect the Minecraft: Bedrock Edition
	 * game protocol version - clients using an incompatible MCPE protocol can still be disconnected afterwards.
	 *
	 * @phpstan-var int[]
	 */
	private const MCPE_RAKNET_EXTRA_ACCEPTED_PROTOCOL_VERSIONS = [8];
	...
	public function __construct(
		...
		$this->rakLib = new RakLibServer(
			...
			$sleeperEntry,
			self::MCPE_RAKNET_EXTRA_ACCEPTED_PROTOCOL_VERSIONS
		);
