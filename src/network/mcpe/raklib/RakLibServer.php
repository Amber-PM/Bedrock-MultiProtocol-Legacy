...
class RakLibServer extends Thread{
	...
	/** @phpstan-var NonThreadSafeValue<int[]> */
	protected NonThreadSafeValue $extraProtocolVersions;

	/**
	 * @phpstan-param ThreadSafeArray<int, string> $mainToThreadBuffer
	 * @phpstan-param ThreadSafeArray<int, string> $threadToMainBuffer
	 * @phpstan-param int[] $extraProtocolVersions
	 */
	public function __construct(
		...
		array $extraProtocolVersions = []
	){
		$this->mainPath = \pocketmine\PATH;
		$this->address = new NonThreadSafeValue($address);
		$this->extraProtocolVersions = new NonThreadSafeValue($extraProtocolVersions);
	}
	...
	protected function onRun() : void{
		...
		$manager = new Server(
			...
			$socket,
			$this->maxMtuSize,
			new MultiVersionProtocolAcceptor($this->protocolVersion, $this->extraProtocolVersions->deserialize()),
