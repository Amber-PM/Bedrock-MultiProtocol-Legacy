...
class InGamePacketHandler extends PacketHandler{
	...
	protected ?Vector3 $lastMove223Position = null;
	protected ?float $lastMove223Yaw = null;
	protected ?float $lastMove223Pitch = null;
	...
	/**
	 * On protocols >= PlayerAuthInputPacket, MovePlayerPacket serverbound is just a noisy
	 * landing-debug echo (real movement comes from handlePlayerAuthInput()) and is ignored
	 * here without side effects.
	 *
	 * On protocol 223, which has no PlayerAuthInputPacket, this is the client's only
	 * movement/rotation source, so it gets the reduced version of the position/rotation/
	 * teleport-lock logic from handlePlayerAuthInput() above.
	 */
	public function handleMovePlayer(MovePlayerPacket $packet) : bool{
		if($this->session->getProtocolId() !== ProtocolInfo::PROTOCOL_1_2_13){
			return true;
		}
		...
		if($rawYaw !== $this->lastMove223Yaw || $rawPitch !== $this->lastMove223Pitch){
			$this->lastMove223Yaw = $rawYaw;
			$this->lastMove223Pitch = $rawPitch;
		...
		$hasMoved = $this->lastMove223Position === null || !$this->lastMove223Position->equals($rawPos);
		$newPos = $rawPos->subtract(0, 1.62, 0)->round(4);
		...
		if(!$this->forceMoveSync && $hasMoved){
			$this->lastMove223Position = $rawPos;
			$this->player->handleMovement($newPos);
		}
	...
	public function handleInventoryTransaction(InventoryTransactionPacket $packet) : bool{
		...
		if($packet->trData instanceof NormalTransactionData){
			if($this->session->getProtocolId() === ProtocolInfo::PROTOCOL_1_2_13){
				//Protocol 223 (1.2.13) predates ItemStackRequestPacket: the legacy client routes *every* inventory
				//interaction (moving items, taking/returning items via the creative menu, dropping items, etc.)
				//through this single NormalTransactionData packet type, using different NetworkInventoryAction
				//source/magic-slot combinations to distinguish them. The modern handleNormalTransaction() below only
				//recognises the narrow SOURCE_WORLD+SOURCE_CONTAINER pair modern clients still send for the Q-drop
				//shortcut, so any other legacy combination (e.g. taking an item from the creative menu, which sends
				//a SOURCE_CONTAINER action plus a SOURCE_CREATIVE action) is rejected with
				//"Unexpected inventory action type ... in drop item transaction". Route protocol 223 through the
				//generic multi-action InventoryTransaction engine instead, mirroring the pre-ItemStackRequest
				//NetworkInventoryAction::createInventoryAction() behaviour this protocol generation expects.
				$result = $this->handleLegacy223NormalTransaction($packet->trData, $packet->requestId);
			}else{
	...
	/**
	 * Protocol 223 (1.2.13) equivalent of the modern ItemStackRequest executor: converts every
	 * NetworkInventoryAction in a legacy NormalTransactionData packet into a core InventoryAction and executes them
	 * together as a single balanced InventoryTransaction, exactly as the pre-ItemStackRequest PocketMine-MP
	 * reference (NetworkInventoryAction::createInventoryAction() + Player::handleInventoryTransaction()) did for
	 * this protocol generation. This covers moving items between slots, taking/returning items via the creative
	 * menu, and dropping items - all of which 1.2.13 clients send through this single packet type.
	 */
	private function handleLegacy223NormalTransaction(NormalTransactionData $data, int $itemStackRequestId) : bool{
		$actions = [];
		//The 1.2.13 client has no concept of ItemStackRequest/recipe IDs - crafting is done purely by
		//moving items in/out of a client-side "crafting grid" fake window via SOURCE_TODO actions (see
		//createLegacy223InventoryAction() below) mixed into the same InventoryTransactionPacket as the ordinary
		//SOURCE_CONTAINER slot changes that place the result into the player's real inventory. The presence of any
		//such action is what distinguishes a crafting attempt from a plain inventory transaction for this protocol
		//generation (modern protocols get this distinction for free from the ItemStackRequest action type instead).
		$isCraftingTransaction = false;
		foreach($data->getActions() as $networkInventoryAction){
			try{
				$action = $this->createLegacy223InventoryAction($networkInventoryAction);
			}catch(\UnexpectedValueException $e){
				$this->session->getLogger()->debug("Unhandled legacy (protocol 223) inventory action: " . $e->getMessage());
				$this->inventoryManager->requestSyncAll();
				return false;
			}
			if($networkInventoryAction->sourceType === NetworkInventoryAction::SOURCE_TODO){
				$isCraftingTransaction = true;
			}
			if($action !== null){
				$actions[] = $action;
			}
		...
		$transaction = $isCraftingTransaction ?
			new CraftingTransaction($this->player, $this->player->getServer()->getCraftingManager(), $actions) :
			new InventoryTransaction($this->player, $actions);

		return $this->executeInventoryTransaction($transaction, $itemStackRequestId);
	}

	/**
	 * Converts a single legacy (protocol 223) NetworkInventoryAction into the equivalent core InventoryAction,
	 * following this protocol generation's source-type/magic-slot semantics:
	 * - SOURCE_CONTAINER: a change to a slot in one of the player's currently open inventories/windows.
	 * - SOURCE_WORLD with magic slot ACTION_MAGIC_SLOT_DROP_ITEM: dropping an item into the world (the "Q" drop
	 *   and the legacy creative/survival "drag out of inventory" drop both use this).
	 * - SOURCE_CREATIVE with magic slot ACTION_MAGIC_SLOT_CREATIVE_CREATE_ITEM: taking an item from the creative
	 *   menu. The item being created is carried in the action's oldItem field.
	 * - SOURCE_CREATIVE with magic slot ACTION_MAGIC_SLOT_CREATIVE_DELETE_ITEM: returning/deleting an item into the
	 *   creative menu. The item being destroyed is carried in the action's newItem field, symmetrically with CREATE.
	 *
	 * - SOURCE_TODO (fake windows for client-side-only UIs):
	 *   - SOURCE_TYPE_CRAFTING_ADD_INGREDIENT / SOURCE_TYPE_CRAFTING_REMOVE_INGREDIENT: a slot change in the
	 *     player's client-side crafting grid (2x2 in the inventory, 3x3 at a crafting table - the client picks the
	 *     grid size itself and always addresses it as this single fake window, regardless of which one is open).
	 *   - SOURCE_TYPE_CRAFTING_RESULT / SOURCE_TYPE_CRAFTING_USE_INGREDIENT: pure markers with no real state of
	 *     their own (the actual ingredient consumption/result placement is expressed via the crafting-grid slot
	 *     changes above and the ordinary SOURCE_CONTAINER change that puts the crafted result into the player's
	 *     real inventory) - these produce no InventoryAction (null), but their presence in the batch is what tells
	 *     the caller to validate the whole batch as a CraftingTransaction instead of a plain balanced one.
	 *   - SOURCE_TYPE_CONTAINER_DROP_CONTENTS: clears a crafting-grid slot without saying which one - found by
	 *     matching oldItem against the grid's current contents.
	 *
	 * @throws \UnexpectedValueException if the action does not correspond to any known/supported legacy pattern
	 */
	private function createLegacy223InventoryAction(NetworkInventoryAction $networkInventoryAction) : ?InventoryAction{
		$typeConverter = $this->session->getTypeConverter();

		switch($networkInventoryAction->sourceType){
			case NetworkInventoryAction::SOURCE_CONTAINER:
				$windowId = $networkInventoryAction->windowId;
				if($windowId === null){
					throw new \UnexpectedValueException("Missing window ID for SOURCE_CONTAINER action");
				}
				$inventoryAndSlot = $this->inventoryManager->locateWindowAndSlot($windowId, $networkInventoryAction->inventorySlot);
				if($inventoryAndSlot === null){
					throw new \UnexpectedValueException("No open inventory for window ID $windowId at slot $networkInventoryAction->inventorySlot");
				}
				[$inventory, $slot] = $inventoryAndSlot;
				return new SlotChangeAction(
					$inventory,
					$slot,
					$typeConverter->netItemStackToCore($networkInventoryAction->oldItem->getItemStack()),
					$typeConverter->netItemStackToCore($networkInventoryAction->newItem->getItemStack())
				);
			case NetworkInventoryAction::SOURCE_WORLD:
				if($networkInventoryAction->inventorySlot !== NetworkInventoryAction::ACTION_MAGIC_SLOT_DROP_ITEM){
					throw new \UnexpectedValueException("Unexpected magic slot $networkInventoryAction->inventorySlot for SOURCE_WORLD action, only drop-item is supported");
				}
				return new DropItemAction($typeConverter->netItemStackToCore($networkInventoryAction->newItem->getItemStack()));
			case NetworkInventoryAction::SOURCE_CREATIVE:
				switch($networkInventoryAction->inventorySlot){
					case NetworkInventoryAction::ACTION_MAGIC_SLOT_CREATIVE_CREATE_ITEM:
						return new CreateItemAction($typeConverter->netItemStackToCore($networkInventoryAction->oldItem->getItemStack()));
					case NetworkInventoryAction::ACTION_MAGIC_SLOT_CREATIVE_DELETE_ITEM:
						return new DestroyItemAction($typeConverter->netItemStackToCore($networkInventoryAction->newItem->getItemStack()));
					default:
						throw new \UnexpectedValueException("Unexpected creative magic slot $networkInventoryAction->inventorySlot");
				}
			case NetworkInventoryAction::SOURCE_TODO:
				$craftingGrid = $this->player->getCraftingGrid();
				switch($networkInventoryAction->windowId){
					...
					case NetworkInventoryAction::SOURCE_TYPE_CRAFTING_REMOVE_INGREDIENT:
						return new SlotChangeAction(
							$craftingGrid,
							$networkInventoryAction->inventorySlot,
							$typeConverter->netItemStackToCore($networkInventoryAction->oldItem->getItemStack()),
							$typeConverter->netItemStackToCore($networkInventoryAction->newItem->getItemStack())
						);
					case NetworkInventoryAction::SOURCE_TYPE_CRAFTING_RESULT:
					case NetworkInventoryAction::SOURCE_TYPE_CRAFTING_USE_INGREDIENT:
						//no-op marker - see docblock above.
						return null;
					case NetworkInventoryAction::SOURCE_TYPE_CONTAINER_DROP_CONTENTS:
						$oldItem = $typeConverter->netItemStackToCore($networkInventoryAction->oldItem->getItemStack());
						$slot = $craftingGrid->first($oldItem, true);
						if($slot === -1){
							throw new \UnexpectedValueException("Fake crafting grid does not contain $oldItem for DROP_CONTENTS action");
						}
						return new SlotChangeAction(
							$craftingGrid,
							$slot,
					...
					default:
						throw new \UnexpectedValueException("Unsupported SOURCE_TODO window ID $networkInventoryAction->windowId for protocol 223");
				}
			default:
				throw new \UnexpectedValueException("Unsupported inventory action source type $networkInventoryAction->sourceType for protocol 223");
		}
	}
	...
	private function handleUseItemTransaction(UseItemTransactionData $data) : bool{
		...
		switch($data->getActionType()){
			...
			case UseItemTransactionData::ACTION_BREAK_BLOCK:
				//Protocol 223 (1.2.13) predates server-authoritative block breaking (there is no
				//PlayerActionPacket::PREDICT_DESTROY_BLOCK on this protocol - see the reference
				//InventoryTransactionPacket::USE_ITEM_ACTION_BREAK_BLOCK handling in legacy PocketMine-MP). The
				//legacy client calculates its own break time (instantly in creative) and, once finished, removes
				//the block from its own local view immediately for feedback, then sends this ACTION_BREAK_BLOCK to
				//tell the server the break happened - in both survival and creative.
				//
				//Until now this action type had no case here (only ACTION_CLICK_BLOCK/ACTION_CLICK_AIR were
				//handled), so it fell through to "return false" below and breakBlock() was never called: the
				//server-side block was left completely untouched in both gamemodes, even though the legacy client
				//had already deleted it from its own render (and simulated any adjacent water flowing into the
				//resulting hole client-side) - explaining both "can't break blocks" and the hole that appears
				//after clicking. If the server rejects the break, we resync the area so the client's incorrect
				//local prediction gets corrected back to the real block.
				if($this->session->getProtocolId() === ProtocolInfo::PROTOCOL_1_2_13){
					self::validateFacing($data->getFace());
					$breakBlockPos = $data->getBlockPosition();
					$vBreakBlockPos = new Vector3($breakBlockPos->getX(), $breakBlockPos->getY(), $breakBlockPos->getZ());
					if(!$this->player->breakBlock($vBreakBlockPos)){
						$this->syncBlocksNearby($vBreakBlockPos, $data->getFace());
					}
					return true;
				}
				return false;
	...
	private function handlePlayerActionFromData(int $action, BlockPosition $blockPosition, int $face) : bool{
		...
		switch($action){
			...
			case PlayerAction::JUMP:
				//legacy (pre-PlayerAuthInputPacket) clients report jumping via PlayerActionPacket instead of the
				//input flags used by modern clients - same pattern as START_SPRINT/START_SNEAK below. This case was
				//previously missing, which caused every jump from a protocol 223 client to be logged as
				//"Unhandled/unknown player action type 8" and the packet rejected.
				$this->player->jump();
				break;
			...
			case PlayerAction::START_SPRINT:
				//legacy (pre-PlayerAuthInputPacket sprint-flag) clients toggle sprinting via PlayerActionPacket
				//instead of the input flags used by modern clients
