...
final class SessionStartPacketHandler extends PacketHandler{
	...
	public function handleRequestNetworkSettings(RequestNetworkSettingsPacket $packet) : bool{
		...
		$this->session->getLogger()->info("Client connecting with protocol version: " . $protocolVersion);
	...
	public function handleLogin(LoginPacket $packet) : bool{
		if($packet->protocol !== ProtocolInfo::PROTOCOL_1_2_13){
			$this->session->disconnectIncompatibleProtocol($packet->protocol);

			return true;
		}
		$this->session->getLogger()->info("Client connecting with protocol version: " . $packet->protocol);
		$this->session->setProtocolId($packet->protocol);
		($this->onSuccess)();

		$handler = $this->session->getHandler();

		return $handler !== null && $handler->handleLogin($packet);
	}
