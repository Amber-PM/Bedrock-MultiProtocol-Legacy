...
class TypeConverter{
	...
	public function __construct(int $protocolId){
		...
		//Protocol 223 (1.2.13) never had an item type dictionary handshake on the wire and uses raw
		//legacy numeric IDs instead of the modern (1.20.0+) arbitrary dictionary runtime IDs. Loading
		//it through ItemTypeDictionaryFromDataHelper, like every other protocol does, would hand legacy
		//clients the wrong item IDs - see Legacy223ItemTypeDictionary's docblock for the full picture.
		$this->itemTypeDictionary = $protocolId === ProtocolInfo::PROTOCOL_1_2_13
			? Legacy223ItemTypeDictionary::get()
			: ItemTypeDictionaryFromDataHelper::loadFromProtocolId($protocolId);
		$this->itemDataDowngrader = new ItemIdMetaDowngrader($this->itemTypeDictionary, ItemTranslator::getItemSchemaId($protocolId));

		//Shields did not exist yet in 1.2.13, so they have no entry in Legacy223ItemTypeDictionary.
		//Fall back to a sentinel value that can never match a real network item ID for that
		//protocol, instead of throwing during session setup.
		try{
			$this->shieldRuntimeId = $this->itemTypeDictionary->fromStringId(ItemTypeNames::SHIELD);
		}catch(\InvalidArgumentException){
			$this->shieldRuntimeId = -1;
		}
		...
		$this->itemTranslator = new ItemTranslator(
			...
			//see ItemTranslator::fromNetworkId() - protocol 223 has no network block-runtime-ID
			//concept and needs the legacy block id/meta upgrade table instead.
			isLegacy223IdSpace: $protocolId === ProtocolInfo::PROTOCOL_1_2_13,
		);
