...
final class CraftingDataCache{
	...
	private function buildCraftingDataCache(CraftingManager $manager) : CraftingDataPacket{
		...
		/**
		 * A recipe ingredient tag (e.g. "any plank", "any wool") is encoded as a TagItemDescriptor by
		 * TypeConverter::coreRecipeIngredientToNet() regardless of protocol - see that method's
		 * TagWildcardRecipeIngredient branch. For protocol 223, CommonTypes::putRecipeIngredient() then hits
		 * its "$descriptor is not IntIdMetaItemDescriptor" case and silently writes that ingredient slot as
		 * an empty/air ItemStack, because the legacy 1.2.13 recipe-ingredient wire format (a plain Slot) has
		 * no concept of a tag at all - there's nowhere to put "any plank" on the wire. That would produce
		 * recipes whose real, non-empty ingredient slot shows up as air to the client: exactly the kind of
		 * malformed recipe entry the ShapelessRecipeType/FurnaceType filters below already guard against
		 * for UIs/recipe kinds that don't exist yet on this client. Skip these the same way, rather than
		 * sending a recipe the legacy crafting/recipe-search UI can't represent correctly.
		 */
		$hasUnrepresentableTagIngredient223 = static function(iterable $ingredients) : bool{
		...
		foreach($manager->getCraftingRecipeIndex() as $index => $recipe){
			...
			if($recipe instanceof ShapelessRecipe){
				//Protocol 223 (1.2.13-1.2.16) predates the stonecutter, cartography table, and smithing
				//table entirely, and its legacy CraftingDataPacket layout has no field to say which block
				//a recipe belongs to anyway (see ShapelessRecipe::encode() legacy branch) - sending these
				//would just clutter/mislead the legacy crafting search with recipes for UIs that don't
				//exist on that client, so skip anything but plain crafting-table recipes.
				if($this->protocolId === ProtocolInfo::PROTOCOL_1_2_13 && $recipe->getType() !== ShapelessRecipeType::CRAFTING){
					continue;
				}
				if($this->protocolId === ProtocolInfo::PROTOCOL_1_2_13 && $hasUnrepresentableTagIngredient223($recipe->getIngredientList())){
					continue;
				}
				...
				foreach($itemTagDowngrader->downgradeShapedRecipe($recipe) as $r){
					if($this->protocolId === ProtocolInfo::PROTOCOL_1_2_13 && $hasUnrepresentableTagIngredient223($r->getIngredientList())){
						continue;
					}
					try{
						$inputs = [];
		...
		foreach(FurnaceType::cases() as $furnaceType){
			//Protocol 223 only ever had the classic furnace - blast furnace, smoker, campfire and soul
			//campfire were all added in later updates the 1.2.13-1.2.16 client never shipped.
			if($this->protocolId === ProtocolInfo::PROTOCOL_1_2_13 && $furnaceType !== FurnaceType::FURNACE){
				continue;
			}
