...
class Player extends Human implements CommandSender, ChunkListener, IPlayer, NeverSavedWithChunkEntity{
	...
	public function setCreativeInventory(CreativeInventory $inventory) : void{
		...
		if($this->spawned && $this->isConnected()){
			//Go through NetworkSession::syncCreativeInventory(), not InventoryManager::syncCreative()
			//directly - the latter always builds the modern CreativeContentPacket, which protocol 223 cannot
			//receive (see that method's doc comment).
			$this->getNetworkSession()->syncCreativeInventory();
		}
	}
