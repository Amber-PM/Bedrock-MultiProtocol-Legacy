...
class Human extends Living implements ProjectileSource, InventoryHolder{
	...
	protected function sendSpawnPacket(Player $player) : void{
		...
		//Mobs are never part of the persistent tab list, so their entry only exists transiently around
		//the spawn packet (added here, removed at the end of this method). Real players are expected to
		//already have a persistent tab-list entry from an earlier NetworkSession::onPlayerAdded()/
		//syncPlayerList() call, so this method normally skips re-sending it for them.
		//
		//Not validated dynamically against a real legacy client: on the legacy 1.2.13-1.2.16 protocol
		//(wire protocol 223), AddPlayerPacket is
		//useless to the client unless the UUID is already present in its player list, and we have no proof
		//that the earlier tab-list broadcast is guaranteed to land before this spawn packet for every join
		//order/timing. Re-sending the (idempotent) ADD entry immediately before AddPlayerPacket closes that
		//gap. This is a no-op for a client that already has the entry, so it's safe to always do for
		//protocol 223 specifically; modern clients keep the previous (already-working) behaviour untouched.
		$isLegacyProtocol = $networkSession->getProtocolId() === ProtocolInfo::PROTOCOL_1_2_13;
		$needsTemporaryListEntry = !($this instanceof Player);

		//DIAGNOSTIC ONLY (temporary - remove once cross-protocol player visibility is confirmed working and
		//log-verified): the only way to know, without a live legacy client, whether this code path is even
		//reached and whether the packets are actually being handed to the send pipeline (as opposed to being
		//silently dropped further down, e.g. by the LEGACY_PROTOCOL_MAX_KNOWN_PACKET_ID safety net or a
		//DataPacketSendEvent listener) is to log it explicitly, in the same style as the existing
		//"[legacy protocol] ..." debug lines in NetworkSession. This block intentionally only fires for the
		//real-player-to-legacy-client case, since that's the one currently reported as broken.
		if($isLegacyProtocol && $this instanceof Player){
			$networkSession->getLogger()->debug(
				"[legacy protocol] sendSpawnPacket for player actor '" . $this->getName() . "' (uuid=" . $this->uuid->toString() . ", runtimeId=" . $this->getId() . ") to legacy session"
			);
		}

		if($needsTemporaryListEntry || $isLegacyProtocol){
			$listAddResult = $networkSession->sendDataPacket(PlayerListPacket::add([PlayerListEntry::createAdditionEntry($this->uuid, $this->id, $this->getName(), $typeConverter->getSkinAdapter()->toSkinData($this->skin))]));
			if($isLegacyProtocol && $this instanceof Player){
				$networkSession->getLogger()->debug("[legacy protocol] PlayerListPacket::add for '" . $this->getName() . "' sendDataPacket() returned " . ($listAddResult ? "true" : "false (DROPPED)"));
			}
		}

		$addPlayerResult = $networkSession->sendDataPacket(AddPlayerPacket::create(
			$this->getUniqueId(),
			$this->getName(),
			$this->getId(),
			"",
		...
		if($isLegacyProtocol && $this instanceof Player){
			$networkSession->getLogger()->debug("[legacy protocol] AddPlayerPacket for '" . $this->getName() . "' sendDataPacket() returned " . ($addPlayerResult ? "true" : "false (DROPPED)"));
		}

		//DATA_NAMETAG is ignored inside AddPlayerPacket on this protocol, so it has to be sent separately
